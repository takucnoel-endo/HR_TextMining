...
data <- read_excel('CleanCorpus.xlsx')
...
stop_w <- pull(read_excel('stopwords.xlsx', col_names=FALSE))
...
syn = read_csv('Synonyms - Sheet1.csv')
...
trinity_jobs <- read_excel("CleanCorpus.xlsx")
...
# Importance = 2.5
# Level = 3.5
bright_skills <- bright_skills%>%
  filter(ImportanceDV>=2.5 & LevelDV>=3.5)

# filter for a specific Occupation to see what their bright skills are
bright_skills%>%
  dplyr::select(Occupation,Skill,ImportanceDV,LevelDV)%>%
  filter(Occupation=="Accountants and Auditors")
...
#b<- left_join(a,SkillFrequency,by="JobCode")
#names(bright_skills)[names(bright_skills)=="SOC Code"]<-"SOC.Code"
unique(trinity_jobs$DEPARTMENT)
trinity_jobs%>%
  dplyr::select(`SOC Code`,JOBCODE)%>%
  filter(JOBCODE=="462")

w<-SkillFrequency%>%
  filter(JobCode=="462")

unique(trinity_jobs$`SOC Code`)
unique(trinity_jobs$`SOC Code`)


z<-gather(w, "Skill", Count, "Active Learning":"Time Management")

x<-left_join(e,z,by="Skill")

e<-bright_skills%>%
  dplyr::select(SOC.Code,Skill)%>%
  filter(SOC.Code=="21-1012.00")

### ^^ this is what we need for every single Trinity job code
# from this info, wed be like "you don't have this bright skill but you do have these"
